...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    usuarioNuevo.twoFactorSecret = secret.base32;
    usuarioNuevo.twoFactorEnabled = true;
    ...
    res.json({
      ...
      twoFactorSetup: {
        ...
        qrCode: qrCodeUrl,
        instrucciones: "Escanea el código QR con tu aplicación de autenticación (Google Authenticator, Authy, etc.) para configurar la autenticación de dos factores"
      }
    });
  } catch (error) {
    res.status(500).json({ error: "Error en el servidor" });
